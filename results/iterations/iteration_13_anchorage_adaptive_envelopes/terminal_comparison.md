# 预测码头与真实码头对比：iteration_13_anchorage_adaptive_envelopes
...
| 预测面积大于真实面积 | 296 (61.4%) |
| 面积比中位数 | 1.522 |
| 面积比 P25 / P75 | 0.587 / 2.985 |
| 面积在真实面积 0.5--2 倍内 | 192 (39.8%) |
...
| 指标 | 数值 |
| --- | ---: |
| 与至少一个真实面相交的预测码头 | 558 / 738 |
| 被至少一个预测面相交的真实码头 | 637 / 922 |
| 预测面覆盖精度（全局并集） | 0.476 |
| 真实面覆盖召回（全局并集） | 0.115 |
| 面范围 IoU（全局并集） | 0.102 |
| 单个真实码头覆盖率中位数 | 0.382 |
...
| 指标 | 计算方式 | 结果 |
| --- | --- | ---: |
| 预测合格率 | 至少与一个真实面有效相交的预测码头 / 全部预测码头 | 558 / 738 (75.6%) |
| 真实码头发现率 | 至少与一个预测面有效相交的真实码头 / 全部真实码头 | 637 / 922 (69.1%) |
| 合格空间关系对 | 所有有效相交的预测/真实面组合 | 810 |
